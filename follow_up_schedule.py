    ...
    while input_item != '-1':
            ...
            if header not in ['Next_Follow_Up_Date' ,'File_No']:
                ...
                if header in ["Follow Up Freq", "Priority"]:
                    if header == 'Follow Up Freq':
                        ...
                        input_item = input(f'{header} (a whole number): ').strip()
        ...
        if not duplicate:
            ...
            add_confirmation = input(
                '\033[96mTo add the above information to the follow up schedule (Y/N)?: \033[0m')
            print('')
            ...
            if add_confirmation.lower() == 'y':
                ...
                print('')
                input_item = input(
                    '\033[96mHit enter to add another or enter -1 to go to main menu: \033[0m')
...
def delete_patient(search_result, delete_selection, follow_up_schedule, filtered_follow_up_schedule):
    ...
    while delete_selection not in ['-1','']:

        deleted_flag = False
        deleted_list = []
        to_delete_list = delete_selection.split(',')
        to_delete_list = [item.strip() for item in to_delete_list] # Creates list of file_no/s to delete
        ...
        if to_delete_list: 
            ...
            if deleted_flag:
                ...
                if filtered_follow_up_schedule != []:
                    delete_selection = input(
                        '\033[96mYou can delete one ore more records by selecting file numbers separated by comma\n\
        ...
        if not set(to_delete_list).issubset(set(search_result)):
            delete_selection = input(
                f'\033[91mPlease enter one of the file numbers above or enter -1 to go to main menu: \033[,5,60m')
